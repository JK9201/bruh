...
from botexe.Database.afk import *
import time
...
@Ubot.on_message(filters.command('afk', hl) & filters.me)
...
@Ubot.on_message(group=afk_watcher)
async def afk(client, message):
  if not await is_afk():
    return
  ...
  if message.chat.id > 0:
    txt = "ɪ ᴀᴍ ᴀꜰᴋ, ꜱɪɴᴄᴇ" + f' `{grt(int(time.time() - det[0]))}`.'
    if det[1]:
      txt += "\n\n"
      txt += "reason :" + f" `{det[1]}`"
